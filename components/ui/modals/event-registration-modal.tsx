...
interface EventBookingResponse {
  message: string;
  data: string; // Paystack URL for paid events, success message for free events
  status: string;
  expiresIn?: number;
  ...
  const handleRegistration = async (data: RegistrationFormData) => {
    ...
    try {
      ...
      if (response.status === "success") {
        if (isFreeEvent) {
          // For free events, close modal and trigger success page
          toast.success("Event booked successfully!");
          onClose();

          // Redirect with booking=free parameter, preserving existing params like type=events
          const currentUrl = new URL(window.location.href);
          const params = new URLSearchParams(currentUrl.search);
          params.set("booking", "free");
