...
export function CreateStaysModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateStaysModalProps) {
  ...
  const createStay = async (): Promise<string> => {
    ...
    try {
      ...
      const validationErrors = StaysService.validateStayData(
        formData as StaysFormData
      );
  ...
  const addUnitsToStay = async (stayId: string): Promise<void> => {
    ...
    if (validationResults.length > 0) {
      const errors = validationResults
        .map((result) => `${result.unitTitle}: ${result.errors.join(", ")}`)
        .join("; ");
    ...
    setSteps((prev) => ({
      ...
      addUnits: {
        status: "loading",
        message: `Adding ${units.length} unit(s)...`,
      },
    }));

    try {
      // Using the UnitsService to create units with progress tracking
  ...
  const isComplete =
    ...
    steps.createStay.status === "success" &&
    steps.addUnits.status === "success";
  const hasError =
    steps.createStay.status === "error" || steps.addUnits.status === "error";
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center space-y-6 w-full">
          ...
          >
            {isComplete
              ? "🎉 Stays Created Successfully!"
              : "Creating Your Accommodation"}
          </h2>
          ...
          <div className="w-full space-y-4">
            ...
            <div className="flex items-center gap-4 p-4 bg-white/40 rounded-lg">
              ...
              <div className="flex-1 text-left">
                ...
                >
                  {steps.createStay.message ||
                    "Setting up basic accommodation details"}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 p-4 bg-white/40 rounded-lg">
              ...
              <div className="flex-1 text-left">
                ...
                <p
                  ...
                  style={{ fontFamily: "var(--font-source-sans), sans-serif" }}
                >
                  {steps.addUnits.message ||
                    `Preparing to add ${units.length} unit(s)`}
                </p>
              </div>
            </div>
          </div>
          ...
          {isComplete && (
            <div className="w-full p-4 bg-green-50 border border-green-200 rounded-lg">
              <p
                ...
                style={{ fontFamily: "var(--font-source-sans), sans-serif" }}
              >
                Your accommodation has been created successfully! You'll be
                redirected shortly.
              </p>
            </div>
          )}
